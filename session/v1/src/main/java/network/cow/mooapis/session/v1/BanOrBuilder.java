// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: cow/session/v1/session_service_api.proto

package network.cow.mooapis.session.v1;

public interface BanOrBuilder extends
    // @@protoc_insertion_point(interface_extends:cow.session.v1.Ban)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <pre>
   * The unique identifier of the ban.
   * </pre>
   *
   * <code>string id = 1 [json_name = "id"];</code>
   * @return The id.
   */
  java.lang.String getId();
  /**
   * <pre>
   * The unique identifier of the ban.
   * </pre>
   *
   * <code>string id = 1 [json_name = "id"];</code>
   * @return The bytes for id.
   */
  com.google.protobuf.ByteString
      getIdBytes();

  /**
   * <pre>
   * The banned player.
   * </pre>
   *
   * <code>string player_id = 2 [json_name = "playerId"];</code>
   * @return The playerId.
   */
  java.lang.String getPlayerId();
  /**
   * <pre>
   * The banned player.
   * </pre>
   *
   * <code>string player_id = 2 [json_name = "playerId"];</code>
   * @return The bytes for playerId.
   */
  com.google.protobuf.ByteString
      getPlayerIdBytes();

  /**
   * <pre>
   * The reason of the ban.
   * </pre>
   *
   * <code>string reason = 3 [json_name = "reason"];</code>
   * @return The reason.
   */
  java.lang.String getReason();
  /**
   * <pre>
   * The reason of the ban.
   * </pre>
   *
   * <code>string reason = 3 [json_name = "reason"];</code>
   * @return The bytes for reason.
   */
  com.google.protobuf.ByteString
      getReasonBytes();

  /**
   * <pre>
   * The unix timestamp of the ban.
   * </pre>
   *
   * <code>uint64 banned_at = 4 [json_name = "bannedAt"];</code>
   * @return The bannedAt.
   */
  long getBannedAt();

  /**
   * <pre>
   * The duration of this ban in seconds, starting from the banned_at timestamp.
   * </pre>
   *
   * <code>int64 duration = 5 [json_name = "duration"];</code>
   * @return The duration.
   */
  long getDuration();

  /**
   * <pre>
   * The initiator of the ban.
   * </pre>
   *
   * <code>.cow.session.v1.Executor executor = 6 [json_name = "executor"];</code>
   * @return Whether the executor field is set.
   */
  boolean hasExecutor();
  /**
   * <pre>
   * The initiator of the ban.
   * </pre>
   *
   * <code>.cow.session.v1.Executor executor = 6 [json_name = "executor"];</code>
   * @return The executor.
   */
  network.cow.mooapis.session.v1.Executor getExecutor();
  /**
   * <pre>
   * The initiator of the ban.
   * </pre>
   *
   * <code>.cow.session.v1.Executor executor = 6 [json_name = "executor"];</code>
   */
  network.cow.mooapis.session.v1.ExecutorOrBuilder getExecutorOrBuilder();

  /**
   * <pre>
   * The revoke of the ban, if there is one.
   * </pre>
   *
   * <code>.cow.session.v1.BanRevoke revoke = 7 [json_name = "revoke"];</code>
   * @return Whether the revoke field is set.
   */
  boolean hasRevoke();
  /**
   * <pre>
   * The revoke of the ban, if there is one.
   * </pre>
   *
   * <code>.cow.session.v1.BanRevoke revoke = 7 [json_name = "revoke"];</code>
   * @return The revoke.
   */
  network.cow.mooapis.session.v1.BanRevoke getRevoke();
  /**
   * <pre>
   * The revoke of the ban, if there is one.
   * </pre>
   *
   * <code>.cow.session.v1.BanRevoke revoke = 7 [json_name = "revoke"];</code>
   */
  network.cow.mooapis.session.v1.BanRevokeOrBuilder getRevokeOrBuilder();
}
